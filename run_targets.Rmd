...
This workflow currently uses n-1 cores as detected by the {parallel} package. The
GEE steps can not be run using multi-core processes since the processing bottleneck
is not local. 
...
# run the pipeline
tar_make(b_check_for_failed_tasks)
```
