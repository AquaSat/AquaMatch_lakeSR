...
for loc_10k in range(math.ceil(len(locations_subset)/10000)):
  ...
  if "site" in extent:
    ...
    if "1" in dswe:
      print("Starting Landsat 4, 5, 7 altered DSWE1 acquisition for site locations at tile "
        + str(tiles)
        + " and location subset "
        + str(loc_10k))
      locs_out_457_D1 = ls457.map(ref_pull_457_DSWE1_altered).flatten()
      locs_out_457_D1 = locs_out_457_D1.filter(ee.Filter.notNull(["med_Blue"]))
      locs_srname_457_D1 = (proj 
        + "_point_LS457_alt_C2_SRST_DSWE1_" 
        + str(tiles)
        + "_" + str(loc_10k)
        +"_v" + str(date.today()))
      ...
      print("Completed Landsat 4, 5, 7 DSWE 1 altered stack acquisitions for site location at tile "
        + str(tiles)
        + " and location subset "
        + str(loc_10k))
    
    else: print("Not configured to acquire DSWE 1 altered stack for Landsat 4, 5, 7 for sites at this location subset.")
    
    if "3" in dswe:
      print("Starting Landsat 4, 5, 7 DSWE3 altered acquisition for site locations at tile " 
        + str(tiles)        
        + " and location subset "
        + str(loc_10k))
      locs_out_457_D3 = ls457.map(ref_pull_457_DSWE3_altered).flatten()
      locs_out_457_D3 = locs_out_457_D3.filter(ee.Filter.notNull(["med_Blue"]))
      locs_srname_457_D3 = (proj
        + "_point_LS457_alt_C2_SRST_DSWE3_" 
        + str(tiles)
        + "_" + str(loc_10k)
        +"_v" + str(date.today()))
      ...
      print("Completed Landsat 4, 5, 7 DSWE 3 altered stack acquisitions for site location at tile "
        + str(tiles)
        + " and location subset "
        + str(loc_10k))
    
    else: print("Not configured to acquire altered DSWE 3 stack for Landsat 4, 5, 7 for sites at this location subset.")
...
with open(("b_pull_Landsat_SRST_poi/out/L457_altered_stack_ids_v"+str(date.today())+".txt"), "w") as fp:
