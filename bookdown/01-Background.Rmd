...
pipeline contained data for Secchi disc depth (SDD), chlorophyll *a* (chla),
dissolved organic carbon (DOC), and total suspended solids (TSS).
...
-   locations file (`lakeSR_collated_WQP_NWIS_sites_with_NHD_info_xDATEx.csv`)
...
and checks for proper Google Drive folder architecture.
...
(non-CONUS HUCs) files where HUC8s were able to be assigned. This is a very time
consuming group of functions and can take multiple days to run if
`compile_locations` in the config file is set to TRUE.
