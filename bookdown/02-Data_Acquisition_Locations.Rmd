...
(provided in the output file `lakeSR_poi_with_flags_xDATEx.csv` via the column
...
during the development of this workflow).
...
inlcuded in the `lakeSR_poi_with_flags_xDATEx.csv` file:
...
`MonitoringLocationTypeName` for WQP sites and `type_cd` for NWIS sites. These
...
the `whatWQPsites()` query, due to the dynamic nature of the WQP. The siteSR
...
`r format(nrow(a_sites_with_NHD_info), big.mark = ",")` distinct sites in the
