...
between them. Because we use a second-order polynomial, all values outside of
the 1st and 99th percentile values used to define the intermission handoff are
flagged and should be used with extreme caution.
...
from the mission `Satellite to Correct` in Table \@ref(tab:gard-coeff-dem-corr7)
or \@ref(tab:gard-coeff-dem-corr8) and $y$ is the harmonized reflectance value
relative to the mission `Satellite to Harmonize to` in the previously-mentioned
tables. To reduce output data product size, we do not apply these handoffs
within the output data product, but rather provide users the tools to apply the
handoffs to the filtered lakeSR and siteSR data.
