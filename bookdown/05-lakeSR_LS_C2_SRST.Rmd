...
The lakeSR-Landsat data product is a set of tabular datasets, representing the
Landsat surface reflectance (SR) and surface tempearture (ST) data summarized
for the point of inaccessibility (POI) location which is meant to represent
pelagic conditions at a given waterbody (identified by its NHD unique
...
data available from all Landsat missions that met our within-image QA criteria.

The siteSR-Landsat data product is similar, but is acquired at unique sampling
locations as described in \@ref(siteSR-locs).

## Changes from AquaSat v1 and LimnoSat-US

While much of the general architecture from the precedent data products remained
...
![Cartoon example: Locations (and their buffers) are the blue circles and the
...
\@ref(handoff) for additional guidance.
...
and Landsat 8/9 to see these masks in action.
...
-   `add_realistic_mask_457` and `add_realistic_mask_89`: Adds a mask bnad where
    ...
    fine-tuned on terrestrial data and small over-corrections of the surface
    reflectance, especially over dark surfaces, is likely to happen. We
...
No mask or correction for sun glint, a phenomenon that causes unusually high
reflectance values in aquatic environments, is currently incorporated into the
GEE workflow. We address sun glint during our post-hoc filtering process
described in section \@ref(post-hoc-filters).

We track pixel-level instances of the use of the aerosol mask in Landsat 8/9,
the opacity mask in Landsat 4/5/7, and the realistic mask (wholistically and per
band) in our GEE workflow. These tallies represent the number of pixels masked
for each QA procedure prior to summarizing the data within the buffered area.

### Defining water area {#dswe-implementation}

We use an adapted version Dynamic Surface Water Extent (DSWE) algorithm to
define what pixels are water within the buffer of our specific locations. DSWE
was defined in @jones2019 for Landsat Collection 1 and re-implemented for
...
2.  we mask any DSWE values that are not fully illuminated (a value of 0 in the
    ...
    ![Screen shot of code used for implementation of hill shadow to determine
