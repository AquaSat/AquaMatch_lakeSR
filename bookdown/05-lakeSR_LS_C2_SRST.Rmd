...
data available from all Landsat missions that met our QA criteria.

The siteSR-Landsat data product is similar, but is acquired at unique sampling
locations identified in Section INSERT REFERENCE TO FUTURE SECTION ABOUT
COLLATING SITESR LOCS.

## Changes from AquaSat v1 and LimnoSat-US

While much of the general architecture from the precedent data products remained
unchanged, in AquaSat v2 have added a number of features to the remote sensing
...
summarize the changes implemented in this update.
...
-   inclusion of Landsat 4 and Landsat 9. At the time of AquaSat v1 and
    ...
    in 2021, and have been added to the data product.
...
-   masking changes. Many updates were made to assure high quality data through
    ...
    `apply_fill_mask_457`, `apply_fill_mask_89`, `apply_realistic_mask_457`,
    `apply_realistic_mask_89`, `apply_opac_mask` (Landsat 4, 5, 7 only),
    `apply_sr_aero_mask` (Landsat 8, 9 only). The cloud mask was also updated to
...
-   DSWE implementation. We have moved from using the JRC Global Surface Water
    Mapping Layers @pekel2016 to using the Dynamic Surface Water Extent
...
HAND-OFF COEFFICIENTS TO BE UPDATED!** for additional guidance.
