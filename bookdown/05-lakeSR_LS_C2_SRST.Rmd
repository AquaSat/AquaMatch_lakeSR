...
While much of the general architecture in AquaSat v1 has remained unchanged, we
...
-   inclusion of Landsat 4 and Landsat 9. At the time of the initial AquaSat,
    Landsat 4 was not easily integrated with other Landsat missions due to data
    storage differences [@ross2019]. Collection 2 updates allowed for easier
...
-   removing no-data acquisitions before GEE export. In the AquaSat update, we
...
lakeSR can be modified to be run by a user by setting up a new config file and
...
Rivers and may not be applicable in all environments.
